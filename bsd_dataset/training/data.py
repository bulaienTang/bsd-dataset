...
class MyDataset(Dataset):
    def __init__(self, path, split, patch=-1, normalize=None):
        super().__init__()
        ...
        # test or validation set use original size, no patch needed
...
def get_dataloaders(options, num_patches):
    ...
    for split in ["val", "test"]:
        ...
        sampler = None
        dataloader = DataLoader(dataset, batch_size = options.batch_size, num_workers = options.num_workers, sampler = sampler)
        dataloader.num_samples = len(dataset)
        dataloader.num_batches = len(dataloader)
        valTestDataloaders[split] = dataloader
    ...
    for patch in range(num_patches):
        ...
        trainDataloadersList.append(dataloaders)
    
    return trainDataloadersList, valTestDataloaders, input_shape, target_shape
